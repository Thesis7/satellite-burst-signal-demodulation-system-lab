...
for i in range(42):
    plt.scatter(X[i], FITZ_Y[i], color='blue', marker='s')
for i in range(42):
    plt.scatter(X[i], LR_Y[i], color='orange', marker='o')
for i in range(42):
    plt.scatter(X[i], KAY_Y[i], color='green', marker='*')
# for i in range(9):
...
plt.yticks([-0.06, -0.04, -0.02, 0, 0.02, 0.04, 0.06, 0.08],
...
plt.grid(True)
# 保存图表到文件
plt.savefig('img/Estimation_Range_Lab2.png')
plt.show()
